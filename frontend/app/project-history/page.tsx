...
import { ArrowLeft, History, FileText, Tag, Calendar, Menu, Sun, Moon } from "lucide-react";
                ...
                <div className="max-w-4xl mx-auto px-4 md:px-8 py-8 md:py-12">
                    ...
                    <header className="flex items-center justify-between mb-8 md:hidden">
                        ...
                        <div className="flex items-center space-x-2">
                            <div className="w-6 h-6 bg-card-bg border border-card-border/50 rounded flex items-center justify-center overflow-hidden">
                    ...
                    {loading ? (
                        <div className="space-y-4">
                            {[1, 2, 3, 4].map((i) => (
                                <div key={i} className="h-32 bg-card-bg/40 border border-card-border animate-pulse rounded-3xl" />
                            ))}
                        </div>
                    ) : (
                        <div className="relative space-y-6">
                            {/* Timeline Line */}
                            <div className="absolute left-[26px] top-0 bottom-0 w-px bg-card-border hidden md:block" />

                            {history.map((item, idx) => (
                                <div key={idx} className="relative md:pl-16 group">
                                    {/* Timeline Dot */}
                                    <div className="absolute left-[22px] top-8 w-2 h-2 rounded-full bg-indigo-500 ring-8 ring-background hidden md:block group-hover:scale-125 transition-all duration-300" />

                                    <div className="bg-card-bg border border-card-border backdrop-blur-md rounded-3xl p-6 md:p-8 shadow-xl hover:border-indigo-500/30 transition-all duration-300 group-hover:-translate-y-1">
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                                            <div className="flex flex-wrap items-center gap-3">
                                                ...
                                                <h2 className="text-lg md:text-xl font-bold text-foreground group-hover:text-indigo-400 transition-colors">
                                                    {item.task}
                                                </h2>
                                            </div>
