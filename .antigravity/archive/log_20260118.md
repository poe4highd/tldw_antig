# 2026-01-18 开发日志 (Part 27)

## 任务：移除侧边栏不再使用的 "YouTube 发现" 按钮

### 1. 需求背景
- **问题描述**：用户反馈 "YouTube 发现" 按钮目前没有实际用处，且处于禁用状态，建议去掉以简化界面。

### 2. 执行内容
- **组件修改**：修改 `frontend/components/Sidebar.tsx`，从 `menuItems` 中移除 "YouTube 发现" 项。
- **类型修复与简化**：由于移除了唯一的 `disabled` 项目，TypeScript 推导 `menuItems` 不再具有 `disabled` 属性导致渲染循环报错。
- **重构**：简化了 `Sidebar.tsx` 中的菜单渲染逻辑，移除了不再需要的 `item.disabled` 判断分支，直接使用 `Link` 组件进行渲染。
- **清理**：从 `lucide-react` 的导入中移除了不再使用的 `Youtube` 图标。

### 3. 经验总结
- **类型推导灵敏度**：在 React + TypeScript 项目中，修改常量数组的内容可能会改变其推导类型，从而影响到依赖该类型的渲染组件。及时的代码简化不仅能修复错误，还能提高代码可读性。

---

# 2026-01-18 开发日志 (Part 26)

## 任务：规范项目历史格式并同步审计规则

### 1. 需求背景
- **问题描述**：`PROJECT_HISTORY.md` 中同一天可能存在多条记录，原格式 `[YYYY-MM-DD]` 难以区分同一天的不同任务。
- **决定**：将日期格式统一改为 `[YYYY-MM-DD Part X]`，其中 X 对应当日的开发任务序号。并将此规则录入全局协作规范。

### 2. 执行内容
- **历史回溯**：根据 `archive/` 文件名和 `DEV_LOG.md` 的记录，为 `PROJECT_HISTORY.md` 中的所有历史条目补全了 `Part` 编号。
- **规范同步**：在 `docs/ai_agent_dev_preferences_cn.md` 中新增“自动开发审计规范”章节，明确了语言、存储、任务流及记录格式的要求。

### 3. 经验总结
- **版本透明度**：通过在历史记录中引入任务序号，可以更精准地将简报与详细的开发日志 (`archive/log_...`) 关联起来，极大地方便了后续的回溯和复盘。

---

# 2026-01-18 开发日志 (Part 25)

## 任务：GPU 加速性能实测与自适应引擎验证

### 1. 测试内容
- **测试环境**：MacBook Pro (Apple Silicon)。
- **测试视频**：`https://youtu.be/QVBpiuph3rM` (灵修系列教程)。
- **测试指令**：使用新实现的 `mlx-whisper` GPU 加速引擎。

### 2. 测试结果
- **引擎识别**：成功触发 `[GPU 加速]` 逻辑。
- **转录性能**：
    - **总耗时**：47.80 秒。
    - **处理规模**：1138 个语音片段 (Segments)。
    - **结论**：性能对比 CPU 模式有量级提升，单视频处理效率进入“秒级”时代。
- **一致性**：输出格式与原系统完全兼容，无缝对接 LLM 语义段落切分逻辑。

### 3. 经验总结
- **Mac 专用库优势**：在 Apple Silicon 平台上，使用 `mlx-whisper` 等原生支持加速的库比通用库性能强出数倍。

---

# 2026-01-18 开发日志 (Part 24)

## 任务：脚本优化 - batch_reprocess.py 参数化

### 1. 优化内容
- **动态参数控制**：修改 `backend/tests/batch_reprocess.py`，支持通过命令行参数指定重处理视频的数量。
- **默认行为调整**：默认重处理数量调整为 1 个，适应单视频快速闭环验证。
- **健壮性**：增加参数类型检查与使用说明。

---

# 2026-01-18 开发日志 (Part 23)

## 任务：提示词深度迭代 - 强化标题关键词强制替换逻辑

### 1. 现象分析
- **案例**：标题包含“灵修与明白神的旨意”，但正文转录为“零修语迷败神的指引”。
- **病因**：之前的提示词过于强调“忠实原文”，导致 LLM 在面对音近但义远的词组时保守，不敢修正。

### 2. 优化措施
- **加权机制**：引入 `【标题权重优先（CRITICAL）】` 指令。
- **还原逻辑**：要求进行拼音模糊匹配，强制将听力噪音还原为标题关键词。
- **结构引导**：使用明确的上下文边界符，提升 LLM 注意力。

### 3. 经验总结
- **指令冲突处理**：当“忠实度”与“准确度”发生冲突时，必须给出一个明确的优先级。在转录校正赛道，上下文（标题）的权威性应高于原始音轨的字面还原。

---

# 2026-01-18 开发日志 (Part 22)


## 任务：优化 AI 转录校正提示词 (Prompt Engineering)

### 1. 需求与现象
- **问题**：转录后文字中仍然存在大量同音错别字（如“智席” vs “窒息”），且正文未充分利用标题和描述中的关键词。
- **目标**：增强 LLM 对同音字（含不同调）的识别能力，并引入视频上下文辅助校错。

### 2. 解决方案
- **提示词升级**：在 `backend/processor.py` 中更新 `PROMPT`，明确要求：
    - 深度识别同音但不同调的错别字。
    - 优先匹配视频标题和描述中的主题关键词。
    - 确保全文术语一致性。
- **逻辑注入**：修改 `split_into_paragraphs` 函数，构建 `video_context` 并将其作为参数传递给 LLM 提示词。

### 3. 回顾与经验
- **上下文可见性**：LLM 在处理孤立的转录文本时往往缺乏先验知识。将标题作为“视频上下文”提供后，LLM 能显著提升对特定领域名词的校正准确度。
- **鲁棒性处理**：在代码中对 `title` 和 `description` 进行了空值处理，确保在无元数据的情况下系统依然稳定。
