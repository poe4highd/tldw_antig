# 2026-01-20 技术咨询：本地服务停止对生产环境的影响

## 咨询背景
用户询问：如果停止本地前后端服务，是否仅影响新任务提交？Vercel + Supabase 是否仍能提供报告页面播放和浏览？

## 分析结论
停止本地服务（尤其是 `cloudflared` 隧道）**会**导致线上生产环境（`read-tube.com`）无法正常工作：
1. **数据获取中断**：虽然数据持久化在 Supabase 云端，但前端目前通过 `api.read-tube.com`（隧道指向本地）中转获取数据。隧道断开后，报告详情页将无法加载数据。
2. **播放功能受限**：
   - YouTube 原生播放模式可能由于无法加载 API 返回的视频 ID 而失效。
   - “同步音频”模式直接依赖本地 `downloads/` 目录，关机后无法访问。
3. **历史记录失效**：Dashboard 列表通过 API 获取，后端停机后书架将显示空白或报错。

## 建议方案
- 若需实现“离机浏览”，需将 FastAPI 后端部署至云端服务器（如 VPS），并将 `api.read-tube.com` 指向该服务器。
- 或者，在前端实现直接查询 Supabase 的降级逻辑（仅限查询，不含处理）。

---

# 2026-01-20 多语言支持 (i18n) 集成

## 任务背景
为项目增加中英文双语支持，提升国际化体验。默认语言设定为英文，并提供简约的语言切换按钮。

## 实施内容
1. **轻量化 i18n 方案**：
   - 实现了基于 React Context 的 `LanguageProvider`。
   - 使用 JSON 文件 (`en.json`, `zh.json`) 存储词条。
   - 支持 `localStorage` 持久化用户语言偏好。
2. **UI 组件适配**：
   - 新增 `LanguageSwitcher` 组件，集成在营销页导航栏和侧边栏底部。
   - 将营销页 (`page.tsx`) 和侧边栏 (`Sidebar.tsx`) 的硬编码文字全部替换为 `t()` 函数调用。
3. **健壮性修复**：
   - 修复了 `LanguageProvider` 在初始加载时未提供 Context 导致的运行时错误。
   - 补全了误删的 `"use client";` 指令及相关 import。

## 验证结果
- 经浏览器验证，默认显示英文。
- 切换按钮交互流畅，中英切换无残留刷新。
- 刷新页面后能正确识别并应用上次保存的语言。

---

# 2026-01-20 全站多语言支持 (i18n) 最终交付

## 回顾
- **实际改动**：全面重构了前端 7 个核心页面（Marketing, Login, Dashboard, Tasks, Settings, Result, Project-History）及 Sidebar 组件。将所有硬编码中文外迁至 JSON 词典，并实现了基于 Context 的实时切换机制。
- **测试结果**：经自测，语言状态在页面刷新、OAuth 跳转后均能正确保持。清理了注释和 Mock 数据中的残留中文，确保了纯净的双向语言体验。

## 经验
- **关键决策**：采用 React Context 而非重型库（如 react-i18next），对于该单页应用而言更轻量、易调试且零配置成本。
- **架构调整**：将 `LanguageSwitcher` 解耦为独立命名导出组件，方便在不同布局（Navbar vs Sidebar）中按需复用。
- **避坑说明**：JSON 文件的多行匹配与缩进在 AI 编辑时极易出错，最终决定采用全量覆盖写入方式以保证语法正确。

---

# 2026-01-20 清理主页开发入口与重定向逻辑说明

## 任务背景
主页（Marketing Page）中的“开发测试”和“开发者入口”按钮已完成其历史使命，现予以移除以保持用户界面的整洁。同时明确 `noredirect=1` 参数的作用。

## 实施内容
1. **界面精简**：移除 `frontend/app/page.tsx` 中所有指向 `/tasks` 的开发入口。
2. **逻辑确认**：
   - `noredirect=1` 是代码中预留的“防自动跳转”开关。
   - 当用户已登录时，访问 `/` 默认会通过 `supabase.auth.onAuthStateChange` 自动跳转到 `/dashboard`。
   - 携带该参数可以阻断这一跳转，允许已登录用户或开发者在不退出的情况下查看主页。

## 验证结果
- 已确认主页导航栏与页脚不再显示相关按钮。
- 已确认 `noredirect=1` 逻辑在 `page.tsx` 中依然保留以供必要时使用。
