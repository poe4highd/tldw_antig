...
- [x] **布局与视觉精化**：
    - **Logo 背景**：浅色模式下 Logo 容器背景改为纯白色 (`bg-white`)。
    - **卡片背景**：缩略图视图下的视频卡片背景改为主题感知的 `bg-card-bg`，白色模式下为纯白。
    - **2 列视图升级**：统一使用“手机展示模式”，即左侧缩略图 + 右侧三行信息（标题、日期、浏览量），大幅提升多列模式下的信息密度。
    - **文案修改**：项目史记页面的副标题删除了冗余的文件路径后缀，更简洁。

- [x] **搜索体验优化**：
    - 引入 300ms 搜索防抖（Debounce），减少 API 调用频率并消除快速输入时的连续闪烁。
    - **骨架屏重构**：将硬编码的深色占位块改为主题感知的透明灰（`bg-card-bg` / `bg-slate-400/10`），彻底解决白色模式下的“黑条闪烁”现象。
    - **细化视觉**：增加缩略图占位块并配合 `animate-pulse`，让加载过程更平滑且具备内容感。

- [x] **浅色模式视觉细化**：
    - 主标题字体缩小约 20% (`text-3xl`)，行高调整为 `leading-[1.1]`，解决文字截断或拥挤感。
    - 关键词滚动条背景梯度与边框实现主题感知，消除浅色模式下的黑遮罩。
    - **顶部功能按钮适配**：语言切换、任务队列、主题切换及设置按钮均统一使用 `bg-card-bg`，在浅色模式下呈现为半透明白色/浅灰色背景，视觉更轻盈一致。
